<?php
/**
 * WPBakery Element: Mixcloud Embed
 *
 * Embeds Mixcloud audio via WordPress oEmbed.
 *
...
if ( class_exists( 'WPBakeryShortCode' ) ) {
	/**
	 * Class WPBakeryShortCode_Sefwpb_Mixcloud_Embed
	 */
	class WPBakeryShortCode_Sefwpb_Mixcloud_Embed extends WPBakeryShortCode {
		...
		public function element_enqueueing_assets() {
			wp_enqueue_style(
				'sefwpb-mixcloud-embed',
				plugins_url( '/assets/css/mixcloud-embed.css', __FILE__ ),
				[],
		...
		public function content( $atts, $content = '' ) {
			...
			$output .= $this->render_mixcloud_embed( $atts );
			$output .= '</div>';
		...
		private function render_title( $atts ) {
			if ( ! empty( $atts['title'] ) ) {
				return '<h3 class="sefwpb-mixcloud-embed-title">' . esc_html( $atts['title'] ) . '</h3>';
			}
			return '';
		}

		/**
		 * Render the Mixcloud embed using WordPress oEmbed.
		 *
		 * @param array $atts Shortcode attributes.
		 * @return string
		 */
		private function render_mixcloud_embed( $atts ) {
			$url = isset( $atts['url'] ) ? $atts['url'] : '';

			if ( empty( $url ) ) {
				return '<p>' . esc_html__( 'Please provide a valid Mixcloud URL to embed.', 'social-elements-for-wpbakery' ) . '</p>';
			}
			...
			if ( function_exists( 'sefwpb_is_lazy_load_enabled' ) && sefwpb_is_lazy_load_enabled() ) {
				$embed_html = sefwpb_wrap_for_lazy_load( $embed_html, 'mixcloud', $url );
			}

			return '<div class="sefwpb-mixcloud-embed-container">' . $embed_html . '</div>';
		}
	}
}

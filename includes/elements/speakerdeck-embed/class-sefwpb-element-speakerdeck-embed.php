<?php
/**
 * WPBakery Element: Speaker Deck Embed
 *
 * Embeds Speaker Deck presentation via WordPress oEmbed.
 *
...
if ( class_exists( 'WPBakeryShortCode' ) ) {
	/**
	 * Class WPBakeryShortCode_Sefwpb_Speakerdeck_Embed
	 */
	class WPBakeryShortCode_Sefwpb_Speakerdeck_Embed extends WPBakeryShortCode {
		...
		public function element_enqueueing_assets() {
			wp_enqueue_style(
				'sefwpb-speakerdeck-embed',
				plugins_url( '/assets/css/speakerdeck-embed.css', __FILE__ ),
				[],
		...
		public function content( $atts, $content = '' ) {
			...
			$output .= $this->render_speakerdeck_embed( $atts );
			$output .= '</div>';
		 ...
		 *
		 * @param string|int $width Width value from attributes.
		 * @return string
		 */
		...
		private function render_title( $atts ) {
			if ( ! empty( $atts['title'] ) ) {
				return '<h3 class="sefwpb-speakerdeck-embed-title">' . esc_html( $atts['title'] ) . '</h3>';
			}
			return '';
		}

		/**
		 * Render the Speaker Deck embed using oEmbed.
		 *
		 * @param array $atts Shortcode attributes.
		 * @return string
		 */
		private function render_speakerdeck_embed( $atts ) {
			$url = isset( $atts['url'] ) ? $atts['url'] : '';

			if ( empty( $url ) ) {
				return '<p>' . esc_html__( 'Please provide a valid Speaker Deck URL to embed.', 'social-elements-for-wpbakery' ) . '</p>';
			}
			...
			if ( function_exists( 'sefwpb_is_lazy_load_enabled' ) && sefwpb_is_lazy_load_enabled() ) {
				$embed_html = sefwpb_wrap_for_lazy_load( $embed_html, 'speakerdeck', $url );
			}

			return '<div class="sefwpb-speakerdeck-embed-container">' . $embed_html . '</div>';
		}
	}
}

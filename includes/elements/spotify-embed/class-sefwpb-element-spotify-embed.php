<?php
/**
 * WPBakery Element: Spotify Embed
 *
 * Embeds Spotify track, album, or playlist.
 *
...
if ( class_exists( 'WPBakeryShortCode' ) ) {
	/**
	 * Class WPBakeryShortCode_Sefwpb_Spotify_Embed
	 *
	 * Handles the Spotify embed element for WPBakery.
	 */
	class WPBakeryShortCode_Sefwpb_Spotify_Embed extends WPBakeryShortCode {
		...
		public function element_enqueueing_assets() {
			wp_enqueue_style(
				...
				plugins_url( '/assets/css/spotify-embed.css', __FILE__ ),
				[],
		...
		public function content( $atts, $content = '' ) {
			...
			$output   .= $this->render_spotify_embed( $atts );
			$output   .= '</div>';
			return $output;
		}
		...
		private function render_title( $atts ) {
			if ( ! empty( $atts['title'] ) ) {
				return '<h3 class="sefwpb-spotify-embed-title">' . esc_html( $atts['title'] ) . '</h3>';
			}
			return '';
		}

		/**
		 * Render the Spotify embed.
		 *
		 * @param array $atts Shortcode attributes.
		 * @return string
		 */
		private function render_spotify_embed( $atts ) {
			$url = isset( $atts['url'] ) ? $atts['url'] : '';

			if ( empty( $url ) ) {
				return '<p>' . esc_html__( 'Please provide a valid Spotify URL to embed.', 'social-elements-for-wpbakery' ) . '</p>';
			}
			...
			if ( function_exists( 'sefwpb_is_lazy_load_enabled' ) && sefwpb_is_lazy_load_enabled() ) {
				$embed_html = sefwpb_wrap_for_lazy_load( $embed_html, 'spotify', $url );
			}

			return '<div class="sefwpb-spotify-embed-container">' . $embed_html . '</div>';
		}
	}
}

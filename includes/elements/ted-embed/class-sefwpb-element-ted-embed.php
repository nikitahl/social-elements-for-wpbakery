<?php
/**
 * WPBakery Element: TED Embed
 *
 * Embeds a TED talk via WordPress oEmbed.
 *
...
if ( class_exists( 'WPBakeryShortCode' ) ) {
	/**
	 * Class WPBakeryShortCode_Sefwpb_Ted_Embed
	 *
	 * Handles the TED embed element for WPBakery.
	 */
	class WPBakeryShortCode_Sefwpb_Ted_Embed extends WPBakeryShortCode {
		...
		public function element_enqueueing_assets() {
			wp_enqueue_style(
				'sefwpb-ted-embed',
				plugins_url( '/assets/css/ted-embed.css', __FILE__ ),
				[],
		...
		public function content( $atts, $content = '' ) {
			...
			$output .= $this->render_ted_embed( $atts );
			$output .= '</div>';
		...
		private function render_title( $atts ) {
			if ( ! empty( $atts['title'] ) ) {
				return '<h3 class="sefwpb-ted-embed-title">' . esc_html( $atts['title'] ) . '</h3>';
			}
			return '';
		}

		/**
		 * Render the TED embed.
		 *
		 * @param array $atts Shortcode attributes.
		 * @return string
		 */
		private function render_ted_embed( $atts ) {
			$url = isset( $atts['url'] ) ? $atts['url'] : '';

			if ( empty( $url ) ) {
				return '<p>' . esc_html__( 'Please provide a valid TED URL to embed.', 'social-elements-for-wpbakery' ) . '</p>';
			}
			...
			if ( function_exists( 'sefwpb_is_lazy_load_enabled' ) && sefwpb_is_lazy_load_enabled() ) {
				$embed_html = sefwpb_wrap_for_lazy_load( $embed_html, 'ted', $url );
			}

			return '<div class="sefwpb-ted-embed-container">' . $embed_html . '</div>';
		}
	}
}

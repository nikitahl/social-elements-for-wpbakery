...
if ( class_exists( 'WPBakeryShortCode' ) ) {
	/**
	 * Class WPBakeryShortCode_Sefwpb_Vimeo_Embed
	 *
	 * Handles the Vimeo embed element for WPBakery.
	 */
	class WPBakeryShortCode_Sefwpb_Vimeo_Embed extends WPBakeryShortCode {
		...
		public function element_enqueueing_assets() {
			wp_enqueue_style(
				'sefwpb-vimeo-embed',
				plugins_url( '/assets/css/vimeo-embed.css', __FILE__ ),
				[],
		...
		public function content( $atts, $content = '' ) {
			...
			$output   .= $this->render_vimeo_embed( $atts );
			$output   .= '</div>';
			return $output;
		}
		...
		private function render_title( $atts ) {
			if ( ! empty( $atts['title'] ) ) {
				return '<h3 class="sefwpb-vimeo-embed-title">' . esc_html( $atts['title'] ) . '</h3>';
			}
			return '';
		}
		 ...
		 * @return string
		 */
		private function render_vimeo_embed( $atts ) {
			$url = isset( $atts['url'] ) ? $atts['url'] : '';

			if ( empty( $url ) ) {
				return '<p>' . esc_html__( 'Please provide a valid Vimeo URL to embed.', 'social-elements-for-wpbakery' ) . '</p>';
			}
			...
			if ( function_exists( 'sefwpb_is_lazy_load_enabled' ) && sefwpb_is_lazy_load_enabled() ) {
				$embed_html = sefwpb_wrap_for_lazy_load( $embed_html, 'vimeo', $url );
			}

			return '<div class="sefwpb-vimeo-embed-container">' . $embed_html . '</div>';
		}
	}
}

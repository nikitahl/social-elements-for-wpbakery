<?php
/**
 * WPBakery Element: YouTube Embed
 *
...
if ( class_exists( 'WPBakeryShortCode' ) ) {
	/**
	 * Class WPBakeryShortCode_Sefwpb_Youtube_Embed
	 *
	 * Handles the YouTube embed element for WPBakery.
	 */
	class WPBakeryShortCode_Sefwpb_Youtube_Embed extends WPBakeryShortCode {
		...
		public function element_enqueueing_assets() {
			wp_enqueue_style(
				'sefwpb-youtube-embed',
				plugins_url( '/assets/css/youtube-embed.css', __FILE__ ),
				[],
		...
		public function content( $atts, $content = '' ) {
			...
			$output   .= $this->render_youtube_embed( $atts );
			$output   .= '</div>';
			return $output;
		}
		...
		private function render_title( $atts ) {
			if ( ! empty( $atts['title'] ) ) {
				return '<h3 class="sefwpb-youtube-embed-title">' . esc_html( $atts['title'] ) . '</h3>';
			}
			return '';
		}
		 ...
		 * @return string
		 */
		private function render_youtube_embed( $atts ) {
			$url = isset( $atts['url'] ) ? $atts['url'] : '';

			if ( empty( $url ) ) {
				return '<p>' . esc_html__( 'Please provide a valid YouTube URL to embed.', 'social-elements-for-wpbakery' ) . '</p>';
			}
			...
			if ( function_exists( 'sefwpb_is_lazy_load_enabled' ) && sefwpb_is_lazy_load_enabled() ) {
				$embed_html = sefwpb_wrap_for_lazy_load( $embed_html, 'youtube', $url );
			}

			return '<div class="sefwpb-youtube-embed-container">' . $embed_html . '</div>';
		}
	}
}
